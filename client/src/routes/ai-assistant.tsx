...
import { AlertTriangle, Bot, Mic, Send, Trash2, User } from "lucide-react";
import { toast } from "sonner";

export const Route = createFileRoute("/ai-assistant")({
  head: () => ({
    meta: [
      { title: "AI Assistant — Emergency Healthcare Connector" },
      { name: "description", content: "Chat with a medical AI to get instant symptom guidance and next-step advice." },
      { property: "og:title", content: "AI Symptom Assistant" },
      { property: "og:description", content: "Describe your symptoms and get AI guidance." },
    ],
  }),
  component: AiAssistant,
});

import { chatbotService, AssistantResponse } from "@/services/api";
...
const suggestions = [
  ...
  "My child has difficulty breathing.",
  "I have severe headache.",
];

function AiAssistant() {
  const [messages, setMessages] = useState<Msg[]>([
    { role: "ai", response: { type: "MEDICAL", content: "Hi, I'm your Healthcare AI assistant. Describe your symptoms and I'll help you decide what to do next." } },
  ]);
  ...
  const clear = () => {
    setMessages([{ role: "ai", response: { type: "MEDICAL", content: "Chat cleared. How can I help?" } }]);
  ...
  return (
    <Layout>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        <div className="mb-4 rounded-2xl border border-destructive/30 bg-destructive/5 p-4 flex gap-3">
          <AlertTriangle className="w-5 h-5 text-destructive shrink-0 mt-0.5" />
          <p className="text-sm text-foreground">
            This AI provides educational guidance only and is not a substitute for professional medical care. In an emergency, call your local emergency number.
          </p>
        </div>
        ...
        <Card className="flex flex-col h-[65vh] overflow-hidden">
          ...
          <div className="border-t border-border p-4 space-y-3">
            <div className="flex flex-wrap gap-2">
                <button onClick={handleHeartAttackGuidance} className="text-xs px-3 py-1.5 rounded-full bg-accent hover:bg-primary hover:text-primary-foreground transition-colors">
                  What should I do during a suspected heart attack?
                </button>
                <button onClick={handleNearestHospital} className="text-xs px-3 py-1.5 rounded-full bg-accent hover:bg-primary hover:text-primary-foreground transition-colors">
                  Find the nearest real hospital to my current location.
                </button>
                <button onClick={handleEmergencyTips} className="text-xs px-3 py-1.5 rounded-full bg-accent hover:bg-primary hover:text-primary-foreground transition-colors">
                  Give me general guidance for handling a medical emergency.
                </button>
                <button onClick={() => send("My child has difficulty breathing.")} className="text-xs px-3 py-1.5 rounded-full bg-accent hover:bg-primary hover:text-primary-foreground transition-colors">
                  My child has difficulty breathing.
                </button>
            </div>
            <form onSubmit={(e) => { e.preventDefault(); send(input); }} className="flex gap-2">
              <Button type="button" variant="outline" size="icon" onClick={() => toast.info("Voice input coming soon")}>
